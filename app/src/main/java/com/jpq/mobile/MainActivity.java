...
public final class MainActivity extends Activity {
    ...
    private void handDialog(){
        ...
        dialog.setOnShowListener(v->dialog.getButton(-1).setOnClickListener(b->{try{if(CaptureService.current!=null)throw new IllegalArgumentException("请先停止识别，再修改上传设置");String url=endpoint.getText().toString().trim();if(enabled.isChecked()){try(HandUpload validation=new HandUpload(url)) {}}pref.edit().putString("endpoint",url).putBoolean("enabled",enabled.isChecked()).apply();dialog.dismiss();}catch(Exception e){toast(e.getMessage());}}));dialog.show();
    }
