...
public final class Vision implements AutoCloseable {
    ...
    private final Map<String,Mat> targets=new HashMap<>(), smallTargets=new HashMap<>();
    public Vision(Pack pack)throws Exception{
        ...
        try{for(JSONObject d:pack.detectors.values())if(d.getString("type").equals("template.ccoeff_normed.v1")){
            ...
            try{Mat t=preprocess(image,d.getJSONObject("params"));MatOfDouble mean=new MatOfDouble(),sd=new MatOfDouble();Core.meanStdDev(t,mean,sd);double max=0;for(double x:sd.toArray())max=Math.max(max,x);mean.release();sd.release();if(max<1){t.release();throw new IllegalArgumentException("模板缺少图像特征");}targets.put(d.getString("id"),t);if(cornerScoring(d))smallTargets.put(d.getString("id"),halfGray(t));}finally{image.release();}
        }}catch(Exception e){close();throw e;}
    }
    static Mat preprocess(Mat image,JSONObject params)throws JSONException {
        Mat source=image.clone(),out=new Mat();
        try {
            if(params.has("background_normalization")) {
                if(!params.getString("background_normalization").equals("cards.background.yellow_to_white.v1"))throw new JSONException("未知底色处理");
                List<Mat> channels=new ArrayList<>();Core.split(source,channels);Mat yellow=new Mat();
                try{Core.min(channels.get(1),channels.get(2),yellow);Core.max(channels.get(0),yellow,channels.get(0));Core.merge(channels,source);}
                finally{yellow.release();for(Mat c:channels)c.release();}
            }
            String mode=params.getString("preprocess");
            if(mode.equals("color"))source.copyTo(out);
            else {Imgproc.cvtColor(source,out,Imgproc.COLOR_BGR2GRAY);if(mode.equals("otsu"))Imgproc.threshold(out,out,0,255,Imgproc.THRESH_BINARY|Imgproc.THRESH_OTSU);}
            return out;
        }catch(Exception e){out.release();throw e;}finally{source.release();}
    }
    ...
    private static boolean cornerScoring(JSONObject detector)throws JSONException {
        JSONObject params=detector.getJSONObject("params");
        // Older white/yellow packs gain the same verification when the app is upgraded.
        return params.has("identity_scoring")||params.has("background_normalization");
    }
    private static Mat halfGray(Mat image){
        Mat gray=new Mat(),small=new Mat();
        try{if(image.channels()==1)image.copyTo(gray);else Imgproc.cvtColor(image,gray,Imgproc.COLOR_BGR2GRAY);
            Imgproc.resize(gray,small,new Size(),.5,.5,Imgproc.INTER_AREA);return small;
        }finally{gray.release();}
    }
    private static final class PreparedScene {
        final Mat image;Mat small;
        PreparedScene(Mat image){this.image=image;}
        Mat small(){if(small==null)small=halfGray(image);return small;}
        void close(){image.release();if(small!=null)small.release();}
    }
    private List<Hit> match(Mat image,JSONObject d,Rect area,String rank,int limit,Map<String,PreparedScene> cache)throws Exception{
        JSONObject options=d.getJSONObject("params");String key=area.toString()+options.getString("preprocess")+options.optString("background_normalization");
        PreparedScene prepared=cache.get(key);
        if(prepared==null){Mat raw=image.submat(area);try{prepared=new PreparedScene(preprocess(raw,options));cache.put(key,prepared);}finally{raw.release();}}
        Mat scene=prepared.image,scores=new Mat();List<Hit> result=new ArrayList<>();Mat target=targets.get(d.getString("id"));
        try{if(target.cols()>scene.cols()||target.rows()>scene.rows())return result;
            double threshold=d.getJSONObject("params").getDouble("threshold");
            if(cornerScoring(d))return matchCorner(scene,prepared.small(),target,smallTargets.get(d.getString("id")),area,rank,threshold,limit);
            Imgproc.matchTemplate(scene,target,scores,Imgproc.TM_CCOEFF_NORMED);
            for(int i=0;i<limit;i++){Core.MinMaxLocResult mm=Core.minMaxLoc(scores);if(!Double.isFinite(mm.maxVal)||mm.maxVal<threshold)break;int x=(int)mm.maxLoc.x,y=(int)mm.maxLoc.y;result.add(new Hit(rank,new Rect(area.x+x,area.y+y,target.cols(),target.rows()),mm.maxVal));suppress(scores,x,y,target.cols(),target.rows());}
            return result;
        }finally{scores.release();}
    }
    private static void suppress(Mat scores,int x,int y,int width,int height){
        int rx=Math.max(1,width/2),ry=Math.max(1,height/2);
        Rect box=new Rect(Math.max(0,x-rx),Math.max(0,y-ry),Math.min(scores.cols(),x+rx+1)-Math.max(0,x-rx),Math.min(scores.rows(),y+ry+1)-Math.max(0,y-ry));
        Mat part=scores.submat(box);part.setTo(new Scalar(-1));part.release();
    }
    private static List<Hit> matchCorner(Mat scene,Mat smallScene,Mat target,Mat smallTarget,Rect area,String rank,double threshold,int limit){
        Mat coarse=new Mat(),fine=new Mat();List<Hit> result=new ArrayList<>();
        try{
            // Cheap position proposals only; acceptance always uses original color pixels.
            Imgproc.matchTemplate(smallScene,smallTarget,coarse,Imgproc.TM_CCOEFF_NORMED);
            for(int i=0;i<limit;i++){
                Core.MinMaxLocResult peak=Core.minMaxLoc(coarse);if(!Double.isFinite(peak.maxVal)||peak.maxVal<Math.min(.65,threshold))break;
                int sx=(int)peak.maxLoc.x,sy=(int)peak.maxLoc.y;
                int left=Math.max(0,sx*2-4),top=Math.max(0,sy*2-4),right=Math.min(scene.cols(),sx*2+target.cols()+4),bottom=Math.min(scene.rows(),sy*2+target.rows()+4);
                if(right-left>=target.cols()&&bottom-top>=target.rows()){
                    Mat patch=scene.submat(new Rect(left,top,right-left,bottom-top));
                    try{Imgproc.matchTemplate(patch,target,fine,Imgproc.TM_CCOEFF_NORMED);}finally{patch.release();}
                    Core.MinMaxLocResult full=Core.minMaxLoc(fine);
                    if(Double.isFinite(full.maxVal)&&full.maxVal>=threshold){
                        int x=left+(int)full.maxLoc.x,y=top+(int)full.maxLoc.y,split=(int)Math.floor(target.rows()*.62+.5);
                        double digit=partScore(scene,target,x,y,0,split),suit=partScore(scene,target,x,y,split,target.rows());
                        if(Double.isFinite(digit)&&Double.isFinite(suit)&&digit>=threshold&&suit>=threshold)result.add(new Hit(rank,new Rect(area.x+x,area.y+y,target.cols(),target.rows()),(digit+suit)/2));
                    }
                }
                suppress(coarse,sx,sy,smallTarget.cols(),smallTarget.rows());
            }
            return result;
        }finally{coarse.release();fine.release();}
    }
    private static double partScore(Mat scene,Mat target,int x,int y,int top,int bottom){
        Mat patch=scene.submat(new Rect(x,y+top,target.cols(),bottom-top)),glyph=target.rowRange(top,bottom),score=new Mat();
        try{Imgproc.matchTemplate(patch,glyph,score,Imgproc.TM_CCOEFF_NORMED);return score.get(0,0)[0];}
        finally{patch.release();glyph.release();score.release();}
    }
    public Frame analyze(Bitmap bitmap,double[] viewport)throws Exception{return analyze(bitmap,viewport,true);}
    public Frame analyze(Bitmap bitmap,double[] viewport,boolean readHands)throws Exception{return analyze(bitmap,viewport,readHands,false);}
    /** The live client consumes hand identities only while arranging, before confirmation. */
    public Frame analyzeCapture(Bitmap bitmap,double[] viewport,boolean readHands)throws Exception{return analyze(bitmap,viewport,readHands,true);}
    private Frame analyze(Bitmap bitmap,double[] viewport,boolean readHands,boolean handPhaseOnly)throws Exception{
        ...
        try{Utils.bitmapToMat(bitmap,rgba);Mat crop=rgba.submat(new Rect(x,y,cw,ch));Imgproc.cvtColor(crop,bgr,Imgproc.COLOR_RGBA2BGR);crop.release();Mat target=normalized.submat(new Rect((int)pack.content[0],(int)pack.content[1],(int)pack.content[2],(int)pack.content[3]));Imgproc.resize(bgr,target,target.size(),0,0,Imgproc.INTER_LINEAR);target.release();return analyzeSource(normalized,readHands,handPhaseOnly);}finally{rgba.release();bgr.release();normalized.release();}
    }
    public Frame analyzeSource(Mat image)throws Exception{return analyzeSource(image,true);}
    public Frame analyzeSource(Mat image,boolean readHands)throws Exception{return analyzeSource(image,readHands,false);}
    private Frame analyzeSource(Mat image,boolean readHands,boolean handPhaseOnly)throws Exception{
        if(Core.mean(image).val[0]<2&&Core.mean(image).val[1]<2&&Core.mean(image).val[2]<2)throw new IllegalArgumentException("黑屏，暂停确认");
        Map<String,PreparedScene> prepared=new HashMap<>();try{
        ...
        for(int i=0;i<states.length();i++){JSONObject s=states.getJSONObject(i);boolean hit=false;JSONArray ids=s.getJSONArray("detector_ids");for(int j=0;j<ids.length();j++){String id=ids.getString(j);if(!found.containsKey(id))found.put(id,detectState(image,pack.detectors.get(id),prepared));hit|=found.get(id);}if(hit){frame.features.add(s.getString("name"));if(s.getString("purpose").equals("round_end"))frame.end=true;if(s.getString("purpose").equals("new_round_candidate"))frame.start=true;}}
        for(JSONObject r:pack.regions.values()){
            JSONObject sem=r.getJSONObject("semantic");boolean hand=sem.optString("role").equals("hand")&&sem.optString("group").equals("me");if(!hand&&!sem.optString("role").equals("played"))continue;if(hand&&(!readHands||(handPhaseOnly&&(!frame.start||frame.end))))continue;
            Reading reading=new Reading();reading.key=r.getString("id");reading.name=r.getString("name");reading.seat=sem.optString("seat_id","me");Rect area=rect(r.getJSONArray("rect"));
            for(JSONObject d:pack.detectors.values())if(d.getString("type").equals("template.ccoeff_normed.v1")){
                ...
                reading.hits.addAll(match(image,d,area,hand?rs.getString("suit")+":"+rs.getString("rank"):rs.getString("rank"),100,prepared));
            }
        ...
        }finally{for(PreparedScene scene:prepared.values())scene.close();}
    }
    private boolean detectState(Mat image,JSONObject d,Map<String,PreparedScene> prepared)throws Exception{
        if(d.getString("type").equals("template.ccoeff_normed.v1")){Rect area=d.isNull("region_id")?new Rect((int)pack.content[0],(int)pack.content[1],(int)pack.content[2],(int)pack.content[3]):rect(pack.regions.get(d.getString("region_id")).getJSONArray("rect"));return !match(image,d,area,"",1,prepared).isEmpty();}
    ...
    public void close(){for(Mat t:targets.values())t.release();targets.clear();for(Mat t:smallTargets.values())t.release();smallTargets.clear();}
}
