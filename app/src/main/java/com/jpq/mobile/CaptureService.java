...
public final class CaptureService extends Service {
    ...
    public void toggle(){automatic=!automatic;main.post(()->{if(handOverlay!=null)handOverlay.update(automatic,null,automatic?"等待新局 / 完整 13 张手牌":"已暂停");});worker.post(()->{cachedFrame=null;handSnapshots.reset();if(!automatic){handResult.end();if(handUpload!=null)handUpload.cancel();panelStatus="已暂停";}else panelStatus="等待新局 / 完整 13 张手牌";if(ledger!=null)ledger.interrupt();hadGap=true;status=automatic?"继续识别 · 当前局可能不完整":"已暂停";persist();});updateStrip();}
