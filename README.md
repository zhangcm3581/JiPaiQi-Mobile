...
Android 十三水两副牌手牌识别客户端（104 张，无大小王，每手 13 张）。当前版本 0.3.3，Android 8.0+、ARM64；旧版出牌记牌器保留在 `v1` 分支。APK 使用开发测试签名，未发布应用商店。

## 使用

1. 从 `dist/JiPaiQi-Mobile-0.3.3-debug-arm64.apk` 安装。
...
4. 悬浮栏含启动/暂停、设置、说明、退出、收起。拖动可移动，收起按钮至少 48dp，自动吸附右侧并保留当前高度；拖动后松手也吸附右侧。点击向左展开，保持同一高度，靠近底边时仅调整到屏幕内，不跳转屏幕中央。隐藏不等于暂停。
5. 左上角结果面板未启动时不显示，点击启动显示、暂停立即隐藏，再次启动恢复显示。运行时可拖动，点击面板隐藏；长按控制栏“收起”，或主页“显示 / 隐藏服务器面板”恢复；暂停时不能手动打开面板。
